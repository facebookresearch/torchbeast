...
void fill_nest_pb(torchbeast::ArrayNest* nest_pb, nest::Nest<T> nest,
  ...
  std::visit(
      nest::overloaded{
          ...
          [nest_pb, &fill_ndarray_pb](const std::vector<Nest>& v) {
            for (const Nest& n : v) {
              torchbeast::ArrayNest* subnest = nest_pb->add_vector();
          ...
          [nest_pb, &fill_ndarray_pb](const std::map<std::string, Nest>& m) {
            ...
            for (const auto& p : m) {
              torchbeast::ArrayNest& subnest_pb = (*map_pb)[p.first];
...
std::invoke_result_t<Function, torchbeast::NDArray*> nest_pb_to_nest(
    torchbeast::ArrayNest* nest_pb, Function array_to_nest) {
  using Nest = std::invoke_result_t<Function, torchbeast::NDArray*>;
